...
class AsyncFilmService:
    ...
    async def fetch_all_films(cls):
        async with create_engine(config.Config.SQLALCHEMY_DATABASE_URI) as engin:
            async with engin.acquire() as conn:
                query = select([Film])
...
class FilmsListApi(HTTPMethodView):
...
app.add_route(FilmsListApi.as_view(), '/films')
